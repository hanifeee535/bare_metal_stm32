...
void i2c_stop (uint8_t i2c) {
	volatile int tmp = 0;
	 if(i2c == 1)
    {
        tmp = I2C1->SR1;  // Read SR1 to clear any flags
        tmp = I2C1->SR2;  // Read SR2 to clear any flags

        I2C1->CR1 |= 0x200;  // Set the STOP bit to generate a STOP condition
    }
    else if(i2c == 2)
    {
        tmp = I2C2->SR1;  // Read SR1 to clear any flags
        tmp = I2C2->SR2;  // Read SR2 to clear any flags
